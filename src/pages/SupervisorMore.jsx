...
import {
  ...
  ClipboardList, Star, Briefcase, DollarSign
} from "lucide-react";
...
// Exact supervisor More tab contents per spec
const SUPERVISOR_SECTIONS = [
  ...
  },
  {
    ...
    items: [
      ...
      { icon: Settings, label: "Settings", page: "Settings", description: "Supervisor settings" },
...
export default function SupervisorMore() {
  ...
  const sectionsWithPayroll = SUPERVISOR_SECTIONS.map(section => {
      ...
      <div className="bg-[#1a2b4a] text-white">
        <div className="px-4 pt-4 pb-2">
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs font-bold tracking-widest uppercase text-[#c9a227]">SUPERVISOR VIEW</span>
            <button
          ...
          <Link to={createPageUrl("Profile")}>
            <div className="flex items-center gap-4 mb-3">
              <Avatar className="w-14 h-14 border-2 border-[#c9a227]">
                ...
                <AvatarFallback className="bg-[#c9a227] text-[#1a2b4a] text-lg font-bold">{user?.full_name?.charAt(0) || "S"}</AvatarFallback>
              </Avatar>
              <div className="flex-1">
                <h2 className="text-base font-semibold">{user?.full_name || "Loading..."}</h2>
                <p className="text-slate-300 text-sm">{user?.email}</p>
                <Badge className="mt-1 bg-blue-600 text-white text-xs">Supervisor</Badge>
              </div>
